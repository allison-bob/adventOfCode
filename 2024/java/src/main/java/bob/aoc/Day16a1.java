...
public class Day16a1 extends BaseClass<Grid2D<Day16a1.Spot>> {
    ...
    public void solve(Grid2D<Spot> data) {
        ...
        BreadthFirst<String, State> target = new BreadthFirst<>(graph);
        target.setNodeVisitor(this::visit);
        Set<String> ids = target.findAllRoutesNodes(start.getId(), this::isEndNode);
        //int routeCt = target.findAllRoutesCount(start.getId(), this::isEndNode);

        // Display result
        LOG.info("result is {}", ids.size());
        //LOG.info("result is {}", routeCt);
    }
    
    private boolean isEndNode(String nodeID) {
        State node = graph.get(nodeID);
        return node == end;
    }
