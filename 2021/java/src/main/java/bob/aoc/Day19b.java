...
public class Day19b extends BaseClass<Map<String, List<Coord3D>>> {
    ...
    public void solve(Map<String, List<Coord3D>> data) {
        ...
        s.setLocation(new Coord3D(0, 0, 0));
        located.add(s);
        ...
        int maxDist = 0;
        for (Day19Scanner s1 : located) {
            for (Day19Scanner s2 : located) {
                maxDist = Math.max(maxDist, s1.getLocation().manhattan(s2.getLocation()));
            }
        }
        LOG.info("Maximum distance between scanners is {}", maxDist);
    }
