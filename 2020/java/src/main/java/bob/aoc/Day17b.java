...
import bob.data.grid.Grid4D;
import bob.parser.Grid2DParser;
import bob.util.BaseClass;
import java.util.ArrayList;
import java.util.List;

public class Day17b extends BaseClass<Grid2D<Day17State>> {
    ...
    public void solve(Grid2D<Day17State> data) {
        Grid4D<Day17State> currState = new Grid4D<>();
        ...
        for (int i = 0; i < 6; i++) {
            ...
            Grid4D<Day17State> newState = new Grid4D<>();
            for (int w = 0; w < currState.getSize().getW(); w++) {
                for (int z = 0; z < currState.getSize().getZ(); z++) {
                    for (int y = 0; y < currState.getSize().getY(); y++) {
                        ...
                        for (int x = 0; x < currState.getSize().getX(); x++) {
                            Day17State here = currState.get(x, y, z, w);
                            //LOG.debug("n:{}", currState.neighborStream(x, y, z, w).collect(Collectors.toList()));
                            long active = currState.neighborStream(new Coord4D(x, y, z, w))
                                    .filter(s -> s == Day17State.ACTIVE)
                                    .count();
                            //LOG.debug("here({},{},{},{})={}, active={}", x, y, z, w, here, active);
                        ...
                        newState.addRow(new Coord4D(0, y, z, w), row);
                    }
                }
            }
