...
public class Day16b extends BaseClass<Grid2D<Day16Spot>> {
    ...
    public void solve(Grid2D<Day16Spot> map) {
        ...
        for (int x = 0; x < map.getSize().getX(); x++) {
            int total = solveOnce(map, x, -1, Day16Direction.DOWN);
            max = Math.max(max, total);
            total = solveOnce(map, x, map.getSize().getY(), Day16Direction.UP);
            max = Math.max(max, total);
        }
        for (int y = 0; y < map.getSize().getY(); y++) {
            int total = solveOnce(map, -1, y, Day16Direction.RIGHT);
            max = Math.max(max, total);
            total = solveOnce(map, map.getSize().getX(), y, Day16Direction.LEFT);
            max = Math.max(max, total);
        }

        LOG.info("Total of {} spots are energized", max);
    }

    private int solveOnce(Grid2D<Day16Spot> map, int x, int y, Day16Direction initdir) {
        Set<Beam> beamCache = new HashSet<>();
        List<Beam> beams = new ArrayList<>();
        beams.add(new Beam(new Coord2D(x, y), initdir));
        ...
        LOG.debug("{},{}:{} -> {}", x, y, initdir, energized.size());
