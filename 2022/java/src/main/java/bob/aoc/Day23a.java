...
public class Day23a extends BaseClass<Grid2D<Day23Spot>> {
    ...
    public void solve(Grid2D<Day23Spot> data) {
        ...
        long elfct = map.pointStream()
                .filter(p -> p == Day23Spot.ELF)
                .count();
        Coord2D minsize = findMinSize();

        LOG.info("Grid is {}, min size is {}; with {} elves there are {} empty spots", map.getSize(), minsize,
                elfct, ((minsize.getX() * minsize.getY()) - elfct));
    }
