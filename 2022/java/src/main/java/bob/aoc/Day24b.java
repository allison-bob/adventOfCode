...
public class Day24b extends BaseClass<Grid2D<List<Day24Blizzard>>> {
    ...
    public void solve(Grid2D<List<Day24Blizzard>> valley) {
        ...
        for (int x = 0; x < valley.getSize().getX(); x++) {
            if (valley.get(x, 0).isEmpty()) {
                first = new Coord2D(x, 0);
            }
            if (valley.get(x, maxY).isEmpty()) {
                last = new Coord2D(x, maxY);
            }
        }
        ...
        maps.clear();
        maps.add(valley);
        Day24Node result1 = doPath(first, last);
        LOG.info("First pass took {} minutes", result1.getCostToHere());
        
        // Move last to first
        Grid2D<List<Day24Blizzard>> prevend = maps.get(result1.getDepth());
        maps.clear();
        maps.add(prevend);
        Day24Node result2 = doPath(last, first);
        LOG.info("Second pass took {} minutes", result2.getCostToHere());

        // Move first to last
        prevend = maps.get(result2.getDepth());
        maps.clear();
        maps.add(prevend);
        Day24Node result3 = doPath(first, last);
        LOG.info("Third pass took {} minutes", result3.getCostToHere());

        LOG.info("Answer is {}", (result1.getCostToHere() + result2.getCostToHere() + result3.getCostToHere()));
    }
    
    private Day24Node doPath(Coord2D first, Coord2D last) {
        start = first;
        finish = last;
        ...
        return algo.findMinCost(startPt);
    }
