...
public class AStarTest {
    ...
    public void testFindMinCost() {
        ...
        Coord2D end = new Coord2D(4, 4);

        // Convert to a graph
        Graph<Coord2D, TestNode> graph = Grid2DToGraph.convert(TestNode.class, grid, (a, b) -> b);

        // Run the algorithm
        AStar<Coord2D, TestNode> target = new AStar<>(graph);
        target.setNodeVisitor(gsc -> gsc.setEstCostToGoal(end.manhattan(gsc.getId())));
        int result = target.findMinCost(new Coord2D(0, 0), end);
