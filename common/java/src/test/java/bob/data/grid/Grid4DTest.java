...
public class Grid4DTest extends GridTestBase {

    private final Grid4D<GridTestEnum> target = new Grid4D<>();
    ...
    private void loadGrid() {
        ...
        target.getSubgrids().add(loadGrid3());
    }
    ...
    private void checkSize(int x, int y, int z, int w) {
        assertAll("grid size",
                () -> assertEquals(w, getGrid4Size(target), "W"),
                () -> assertEquals(z, getGrid3Size(getGrid3(target, 0)), "Z"),
                () -> assertEquals(y, getGrid2Size(getGrid2(getGrid3(target, 0), 0)), "Y"),
                () -> assertEquals(x, getGrid1Size(getGrid1(getGrid2(getGrid3(target, 0), 0), 0)), "X")
        );
    }
    ...
    public void testAddBorder() {
        ...
        Grid3D<GridTestEnum> bgw0 = getGrid3(target, 0);
        Grid3D<GridTestEnum> bgw2 = getGrid3(target, 2);
        for (int z = 0; z < getGrid3Size(bgw0); z++) {
            Grid2D<GridTestEnum> bgz0 = getGrid2(bgw0, z);
            Grid2D<GridTestEnum> bgz2 = getGrid2(bgw2, z);
            for (int y = 0; y < getGrid2Size(bgz0); y++) {
                Grid1D<GridTestEnum> bgy0 = getGrid1(bgz0, y);
                Grid1D<GridTestEnum> bgy2 = getGrid1(bgz2, y);
                for (int x = 0; x < getGrid1Size(bgy0); x++) {
                    assertEquals(GridTestEnum.BORDER, getGrid0(bgy0, x), "(" + x + "," + y + "," + z + ",0)");
                    assertEquals(GridTestEnum.BORDER, getGrid0(bgy2, x), "(" + x + "," + y + "," + z + ",2)");
                }
            }
        }
    }
    ...
    public void testAddRowEmpty() {
        ...
        checkSize(3, 1, 1, 1);
        Grid1D<GridTestEnum> g0 = getGrid1(getGrid2(getGrid3(target, 0), 0), 0);
        for (int x = 0; x < testRows[0].size(); x++) {
            assertEquals(testRows[0].get(x), getGrid0(g0, x), "(" + x + ",0,0,0)");
        }
    }
    ...
    public void testAddRowNotEmpty() {
        ...
        checkSize(3, 2, 1, 1);
        Grid1D<GridTestEnum> g0 = getGrid1(getGrid2(getGrid3(target, 0), 0), 0);
        Grid1D<GridTestEnum> g1 = getGrid1(getGrid2(getGrid3(target, 0), 0), 1);
        for (int x = 0; x < testRows[0].size(); x++) {
            assertEquals(testRows[0].get(x), getGrid0(g0, x), "(" + x + ",0,0,0)");
            assertEquals(testRows[1].get(x), getGrid0(g1, x), "(" + x + ",1,0,0)");
        }
    }
    ...
    public void testAddRowLNegative() {
        RuntimeException thrown = assertThrows(RuntimeException.class,
                () -> target.addRow(new Coord4D(0, 0, 0, -3), testRows[0]));
    ...
    public void testAddRowLEmptyBadCoord() {
        RuntimeException thrown = assertThrows(RuntimeException.class,
                () -> target.addRow(new Coord4D(0, 0, 0, 1), testRows[0]));
        assertTrue(thrown.getMessage().contains("first row"), "Unexpected exception message");
    }

    @Test
    public void testAddRowLNotEmptyBadCoord() {
        target.addRow(new Coord4D(0, 0, 0, 0), testRows[0]);
        RuntimeException thrown = assertThrows(RuntimeException.class,
                () -> target.addRow(new Coord4D(0, 0, 0, 2), testRows[0]));
    ...
    public void testAddRowLNotEmpty() {
        target.addRow(new Coord4D(0, 0, 0, 0), testRows[0]);
        target.addRow(new Coord4D(0, 0, 0, 1), testRows[1]);

        checkSize(3, 1, 1, 2);
        Grid1D<GridTestEnum> g0 = getGrid1(getGrid2(getGrid3(target, 0), 0), 0);
        Grid1D<GridTestEnum> g1 = getGrid1(getGrid2(getGrid3(target, 1), 0), 0);
        for (int x = 0; x < testRows[0].size(); x++) {
            assertEquals(testRows[0].get(x), getGrid0(g0, x), "(" + x + ",0,0,0)");
            assertEquals(testRows[1].get(x), getGrid0(g1, x), "(" + x + ",0,0,1)");
        }
    }
        ...
        for (int z = 0; z < testRows3D.length; z++) {
            ...
            for (int y = 0; y < planes.length; y++) {
                ...
                for (int x = 0; x < row.size(); x++) {
                    assertEquals(row.get(x), target.get(x, y, z, 0), "(" + x + "," + y + "," + z + ",0)");
                }
            }
        }
    }
    ...
    public void testGetOutside() {
        ...
        assertAll("low coordinate",
                () -> assertNull(target.get(-1, 0, 0, 0), "X"),
                () -> assertNull(target.get(0, -1, 0, 0), "Y"),
                () -> assertNull(target.get(0, 0, -1, 0), "Z"),
                () -> assertNull(target.get(0, 0, 0, -1), "W")
        );
        assertAll("high coordinate",
                () -> assertNull(target.get(3, 0, 0, 0), "X"),
                () -> assertNull(target.get(0, 3, 0, 0), "Y"),
                () -> assertNull(target.get(0, 0, 3, 0), "Z"),
                () -> assertNull(target.get(0, 0, 0, 1), "W")
        );
    }
    ...
    public void testGetSize() {
        ...
        getGrid3(target, 0).getSubgrids().remove(2);
        getGrid1(getGrid2(getGrid3(target, 0), 0), 0).getPoints().add(GridTestEnum.P);
        checkSize(4, 3, 2, 1);

        Coord4D size = target.getSize();
        
        assertAll("grid size",
                () -> assertEquals(1, size.getW(), "W"),
    ...
    public void testNeighborStream() {
        ...
        List<GridTestEnum> neighbors = target.neighborStream(new Coord4D(1, 1, 1, 0))
                .collect(Collectors.toList());
        
        assertEquals(80, neighbors.size(), "count of neighbors");
        long nullCt = neighbors.stream()
                .filter(n -> n == null)
                .count();
        assertEquals(54, nullCt, "number of null neighbors");
    ...
    public void testDirections() {
        ...
        List<Coord4D> dirs = target.directions()
                .distinct()
                .collect(Collectors.toList());
        
        assertEquals(81, dirs.size(), "count of distinct directions to neighbors");
    }
}
